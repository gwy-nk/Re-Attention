from core.data.VQA_load_data import DataSetMC, DataSetOE
...
class Execution:
    def __init__(self, __C):
        ...
        if self.__C.DATASET == "VQA1+OE":
            self.dataset = DataSetOE(self.__C)
            self.task = "OE"
        else:
            self.dataset = DataSetMC(self.__C)
            self.task = "MC"
        ...
        if self.__C.EVAL_EVERY_EPOCH:
            ...
            if self.__C.DATASET == "VQA1+OE":
                self.dataset_eval = DataSetOE(__C_eval)
            else:
                self.dataset_eval = DataSetMC(__C_eval)

    def train(self, dataset, dataset_eval=None):

        from core.model.net import Net
        
        print(self.__C.DATASET, '===============')
        ...
        if self.__C.RESUME:
            ...
            if ('ckpt_' + self.__C.VERSION) not in os.listdir(self.__C.CKPTS_PATH):
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            if self.__C.SHUFFLE_MODE == 'external':
                shuffle_list(dataset.ques_list)
            ...
            for step, (
                    ...
                    ques_ix_iter,
                    ans_iter,
                    img_id, bboxes, ques_id,
                    ans_label, ans_mc, ans_mc_ids,
                    mces
                ...
                loss = loss_fn(pred, ans_iter) + self.__C.recon_rate * recon_loss
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        qid_list = [ques['ques_id'] for ques in dataset.ques_list]
        ans_ix_list = []
        pred_list = []
        ...
        for step, (
            ...
            if self.task == "MC":
                ...
                pred_argmax = np.zeros(pred_np.shape[0], dtype=np.int64)
                for j in range(pred_np.shape[0]):
                    mc_idx = ans_mc_ids.cpu().numpy()[j].tolist()
                    # remove padding
                    mc_idx = [e for e in mc_idx if e != -1]
                    pred_argmax[j] = -1
                    ans_prob = 0
                    for k in range(len(pred_np[j])):
                        if k in mc_idx:
                            if pred_argmax[j] == -1 or ans_prob < pred_np[j][k]:
                                pred_argmax[j] = k
                                ans_prob = pred_np[j][k]
        ...
        for qix in range(qid_list.__len__()):
            if ans_ix_list[qix] == -1:
                result.append(
                    {
                        'answer': "my_wrong_answer",
        ...
        if valid:
            if self.task == "MC":
                ques_file_path = "/ExpData/gwy/vqa/v1/MultipleChoice_mscoco_val2014_questions.json"
            else:
                ques_file_path = "/ExpData/gwy/vqa/v1/OpenEnded_mscoco_val2014_questions.json"
            ans_file_path = "/ExpData/gwy/vqa/v1/mscoco_val2014_annotations.json"
