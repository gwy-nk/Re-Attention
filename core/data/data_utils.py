...
def img_feat_load(path_list, element_name='x'):
    iid_to_feat = {}

    for ix, path in enumerate(path_list):
        iid = str(int(path.split('/')[-1].split('_')[-1].split('.')[0]))
        img_feat = np.load(path)
        img_feat_x = img_feat[element_name].transpose((1, 0))
...
def ques_load(ques_list, element_name='question_id'):
    qid_to_ques = {}

    for ques in ques_list:
        qid = str(ques[element_name])
        qid_to_ques[qid] = ques

    return qid_to_ques


def tokenize(stat_ques_list, use_glove, element_name='question'):
    token_to_ix = {
        'PAD': 0,
        'UNK': 1,
    }
...
def proc_img_feat(img_feat, img_feat_pad_size):
    try:
        ...
        img_feat = np.zeros((100, 2048), dtype=np.float32)
        pass

    return img_feat


def proc_ques(ques, token_to_ix, max_token, element_name='question'):
